...
:: Backup current JSON with timestamp
...
:: Ensure Roblox folder exists
if not exist "%destination_folder%" (
    mkdir "%destination_folder%"
)

:: Copy current JSON to Roblox folder
...
:: Read original JSON into temp file
set temp_json=%~dp0IxpSettings_temp.json
if exist "%temp_json%" del "%temp_json%"
(echo {) >"%temp_json%"

:: Copy original JSON (skip first and last line with braces)
